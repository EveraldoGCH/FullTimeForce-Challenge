...
const Header: React.FC = () => {
    const { dark } = useAppSelector((state) => state.ui)
    ...
    return (
        <header className={!dark?"w-full h-[80px] flex items-center justify-around bg-white":"w-full h-[80px] flex items-center justify-around bg-slate-300"}>
            ...
            <p className="w-[10%] decoration-black font-sans font-semibold m-0">{!dark?"Everaldo Guevara":"Everaldo Guevara!"}</p>
        </header>
    );
}
